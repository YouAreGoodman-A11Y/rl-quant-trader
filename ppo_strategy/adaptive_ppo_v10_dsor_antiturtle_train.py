...
class RLTraderEnvV10(gym.Env):
    def __init__(self, date_list, env_data, is_eval=False):
        super(RLTraderEnvV10, self).__init__()
    ...
    def step(self, action):
        ...
        # 4. Stats & DSoR + Cash Drag Penalty Reward
        ...
        # --- 核心1：计算 DSoR (Differential Sortino Ratio) 奖励 ---
        ...
        # --- 核心2：反乌龟机制 (Cash Drag Penalty) ---
        # 获取当日市场的平均收益率 (仅计算当日未停牌的有效股票)
        market_rt = 0.0
        valid_market_mask = d_env['valid_mask_2d'][d_idx]
        if np.any(valid_market_mask):
            market_rt = float(np.mean(d_env['ret_2d'][d_idx][valid_market_mask]) * 100.0)
            
        cash_ratio = float(self.cash / max(self.total_asset, 1.0))
        
        cash_drag_penalty = 0.0
        # 如果大盘大涨 (涨幅 > 0.5%)，模型却留有现金，则施加严厉惩罚
        if market_rt > 0.5:
            cash_drag_penalty = - (market_rt * cash_ratio * 2.0)
            
        final_reward = dsor_reward + cash_drag_penalty
        reward = float(np.clip(final_reward, -5.0, 5.0))
...
if __name__ == "__main__":
    ...
    print("Initializing Vectorized Train Environment (V10 - DSoR + Anti-Turtle)...")
    env = DummyVecEnv([lambda: RLTraderEnvV10(train_dates, env_data, is_eval=False)])
    print("Training RL Model...")
    model = PPO("MlpPolicy", env, verbose=1, learning_rate=0.0003, ent_coef=0.01, n_steps=1024, batch_size=64)
    model.learn(total_timesteps=80000)
    model.save("/root/.openclaw/workspace-gemini-assistant/rl_trader/ppo_strategy/adaptive_ppo_v10_dsor_antiturtle_model")
