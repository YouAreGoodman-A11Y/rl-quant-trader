...
from adaptive_ppo_v10_dsor_antiturtle_train import load_data, RLTraderEnvV10
...
def plot_v7_standard(records, title, save_path):
    ...
    ax3.plot(df.index, df['DSoR_Reward'], label='DSoR Reward Signal (Base)', color='green', linewidth=1.0, alpha=0.6)
    ax3.plot(df.index, df['Cash_Drag_Penalty'], label='Cash Drag Penalty (Anti-Turtle)', color='red', linewidth=1.5, alpha=0.8)
...
if __name__ == "__main__":
    model_path = "/root/.openclaw/workspace-gemini-assistant/rl_trader/ppo_strategy/adaptive_ppo_v10_dsor_antiturtle_model.zip"
    train_dates, test_dates, env_data = load_data()
    model = PPO.load(model_path)
    
    train_env = RLTraderEnvV10(train_dates, env_data, is_eval=True)
    run_evaluation(model, train_env, "V10 DSoR+AntiTurtle (Train Set)", "/root/.openclaw/workspace-gemini-assistant/rl_trader/ppo_strategy/v10_dsor_antiturtle_train_eval.png")

    test_env = RLTraderEnvV10(test_dates, env_data, is_eval=True)
    run_evaluation(model, test_env, "V10 DSoR+AntiTurtle (Test Set)", "/root/.openclaw/workspace-gemini-assistant/rl_trader/ppo_strategy/v10_dsor_antiturtle_test_eval.png")
