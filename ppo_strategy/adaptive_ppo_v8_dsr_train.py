...
class RLTraderEnvV8(gym.Env):
    def __init__(self, date_list, env_data, is_eval=False):
        super(RLTraderEnvV8, self).__init__()
    ...
    def reset(self, seed=None, options=None):
        ...
        # --- DSR (微分夏普比率) 在线均值和平方均值初始化 ---
        self.dsr_A = 0.0      # EMA of returns
        self.dsr_B = 1e-4     # EMA of squared returns (初始化给个小偏置防除零)
        self.eta = 0.05       # 衰减平滑系数 (约等于过去20天的加权窗口)
    ...
    def step(self, action):
        ...
        # --- 核心：计算 DSR (Differential Sharpe Ratio) 奖励 ---
        # 放大收益率，避免浮点数精度在计算方差时被吃掉
        rt = float(daily_return * 100.0)
        
        delta_A = rt - self.dsr_A
        delta_B = rt**2 - self.dsr_B
        
        variance = self.dsr_B - self.dsr_A**2
        if variance < 1e-6:
            variance = 1e-6
            
        # Moody's DSR Formula 推导的单步贡献度
        dsr_reward = (self.dsr_B * delta_A - 0.5 * self.dsr_A * delta_B) / (variance ** 1.5)
        
        # 状态更新
        self.dsr_A += self.eta * delta_A
        self.dsr_B += self.eta * delta_B
        
        # 防止奖励爆炸，给 PPO 一点稳定性剪裁
        reward = float(np.clip(dsr_reward, -5.0, 5.0))
...
if __name__ == "__main__":
    ...
    print("Initializing Vectorized Train Environment (V8 - DSR Reward)...")
    env = DummyVecEnv([lambda: RLTraderEnvV8(train_dates, env_data, is_eval=False)])
    print("Training RL Model...")
    model = PPO("MlpPolicy", env, verbose=1, learning_rate=0.0003, ent_coef=0.01, n_steps=1024, batch_size=64)
    model.learn(total_timesteps=80000)
    model.save("/root/.openclaw/workspace-gemini-assistant/rl_trader/ppo_strategy/adaptive_ppo_v8_dsr_model")
