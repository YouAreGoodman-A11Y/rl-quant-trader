...
SKIP_TOP_PCT = 0.01  # 固定买入下限: 避开前1%
SELL_RANK_PCT = 0.4  # 固定卖出上限: 跌出前40%卖出
...
def load_data():
    ...
    # 买入条件：排名大于 0.01（避开前1%），非涨停，非停牌，非ST，且当日有效
    buy_cand_mask = (
...
class RLTraderEnvV7(gym.Env):
    def __init__(self, date_list, env_data, is_eval=False):
        super(RLTraderEnvV7, self).__init__()
        self.date_list = date_list
        self.env_data = env_data
        self.is_eval = is_eval
        
        # Action: 一维动作空间，控制目标持股只数
        self.action_space = spaces.Box(low=0.0, high=1.0, shape=(1,), dtype=np.float32)
        # Observation: 8个宏观因子 + 1个水位线 + 1个当前持股集中度
    ...
    def _get_state(self):
        ...
        held_count = np.sum(self.portfolio_values > 0)
        held_z = np.clip((held_count - 500) / 250.0, -3.0, 3.0) 
        
        obs = np.concatenate([macro_obs, [dd_z, held_z]]).astype(np.float32)
        return obs
    ...
    def step(self, action):
        ...
        # AI 动态决定持有只数映射，允许最低30只防崩盘，最高约1000只极度分散
        target_holdings = int(action_val * 970) + 30 
        
        date_str = self.date_list[self.current_step]
        d_idx = self.env_data['date2idx'][date_str]
        d_env = self.env_data
        
        # 1. 固定的卖出逻辑 (排名 > 0.4，且非跌停非停牌)
        ...
        # 只要手头有钱（初始本金或卖出得来的现金），就必须全部打光以维持满仓
        if self.cash > 1.0: 
            num_to_buy = int(target_holdings - num_held)
            # 如果 AI 想收缩持仓（target_holdings < num_held），我们并不强平合格的持仓。
            # 为了维持 100% 满仓，我们强制把现金集中打入头部的 1 只股票。
            if num_to_buy < 1:
                num_to_buy = 1 
...
if __name__ == "__main__":
    ...
    print("Initializing V7 Train Environment...")
    env = DummyVecEnv([lambda: RLTraderEnvV7(train_dates, env_data, is_eval=False)])
    print("Training RL Model (V7 Dynamic Holdings Count)...")
    model = PPO("MlpPolicy", env, verbose=1, learning_rate=0.0003, ent_coef=0.01, n_steps=1024, batch_size=64)
    model.learn(total_timesteps=80000)
    model.save("/root/.openclaw/workspace-gemini-assistant/rl_trader/ppo_strategy/adaptive_ppo_v7_holdcount_model")
