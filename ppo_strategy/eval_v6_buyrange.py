...
class RLTraderEnvV6_Eval:
    ...
    def _get_state(self, current_step, total_asset, peak_asset, cash, holdings):
        ...
        held_z = np.clip((held_count - 250) / 100.0, -3.0, 3.0)
    ...
    def evaluate(self, model):
        ...
        for current_step in range(len(self.step_indices)):
            ...
            val1 = int(np.clip(action[0], 0.0, 1.0) * 1000)
            val2 = int(np.clip(action[1], 0.0, 1.0) * 1000)
            
            start_rank = min(val1, val2)
            end_rank = max(val1, val2)
            if end_rank - start_rank < 30:
...
def plot_dataset(res_df, title_prefix, out_path, color):
    ...
    ax2.fill_between(res_df['Date'], res_df['Start_Rank'], res_df['End_Rank'], color='purple', alpha=0.1, label='Buy Rank Band')
    ax2.set_ylabel('Target Rank Interval [0-1000]', color='purple')
...
if __name__ == '__main__':
    from adaptive_ppo_v6_buyrange import load_numpy_cache
    ...
    print("Loading Model...")
    model_path = "/root/.openclaw/workspace-gemini-assistant/rl_trader/ppo_strategy/adaptive_ppo_v6_buyrange.zip"
    model = PPO.load(model_path)
    
    print("Evaluating Train Set...")
    train_eval = RLTraderEnvV6_Eval(train_idx, train_dates, cache)
    train_res = train_eval.evaluate(model)
    
    print("Evaluating Test Set...")
    test_eval = RLTraderEnvV6_Eval(test_idx, test_dates, cache)
    test_res = test_eval.evaluate(model)
    
    plot_dataset(train_res, "RL V6 - Dynamic Buy Range (Train Set)", "/root/.openclaw/workspace-gemini-assistant/rl_trader/ppo_strategy/rl_v6_train_buyrange.png", "blue")
    plot_dataset(test_res, "RL V6 - Dynamic Buy Range (Test Set)", "/root/.openclaw/workspace-gemini-assistant/rl_trader/ppo_strategy/rl_v6_test_buyrange.png", "red")
