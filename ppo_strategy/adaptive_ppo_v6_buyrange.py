...
# =======================================================
# 2. V6 环境: 2D动作控制动态选股区间 (Buy Range)
# =======================================================
class RLTraderEnvV6(gym.Env):
    def __init__(self, step_indices, dates, cache):
        super(RLTraderEnvV6, self).__init__()
        ...
        # 动作空间：2维连续值，用于映射两个整数区间
        self.action_space = spaces.Box(low=0.0, high=1.0, shape=(2,), dtype=np.float32)
        # 观察空间：8维宏观 + 回撤水位 + 当日胜率(简化替代现金比例，因为现金一直为0) = 10维
    ...
    def step(self, action):
        # AI 输出决定选股区间：将 [0, 1] 映射到 [0, 1000] 名
...
if __name__ == '__main__':
    ...
    print("\nInitializing Vectorized Env V6 (Buy Range)...")
    env = DummyVecEnv([lambda: RLTraderEnvV6(train_idx, train_dates, cache)])
    
    print("Training PPO Model (V6 Buy Range, 80000 steps)...")
    model = PPO("MlpPolicy", env, verbose=1, learning_rate=0.0003, ent_coef=0.01, n_steps=1024, batch_size=64)
    model.learn(total_timesteps=80000)
    
    save_path = "/root/.openclaw/workspace-gemini-assistant/rl_trader/ppo_strategy/adaptive_ppo_v6_buyrange"
