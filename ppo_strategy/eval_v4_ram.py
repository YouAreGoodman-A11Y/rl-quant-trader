...
from adaptive_ppo_v4_ram_train import load_data, RLTraderEnvV4
...
def plot_v7_standard(records, title, save_path):
    ...
    ax3.plot(df.index, df['Holdings_Count'], label='Holdings Count', color='purple', linewidth=1.5)
...
if __name__ == "__main__":
    model_path = "/root/.openclaw/workspace-gemini-assistant/rl_trader/ppo_strategy/adaptive_ppo_v4_ram_model.zip"
    train_dates, test_dates, env_data = load_data()
    model = PPO.load(model_path)
    
    train_env = RLTraderEnvV4(train_dates, env_data, is_eval=True)
    run_evaluation(model, train_env, "V4 RAM (Train Set)", "/root/.openclaw/workspace-gemini-assistant/rl_trader/ppo_strategy/v4_ram_train_eval.png")

    test_env = RLTraderEnvV4(test_dates, env_data, is_eval=True)
    run_evaluation(model, test_env, "V4 RAM (Test Set)", "/root/.openclaw/workspace-gemini-assistant/rl_trader/ppo_strategy/v4_ram_test_eval.png")
