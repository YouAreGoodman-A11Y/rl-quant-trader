...
def load_data():
    ...
    print(f"Matrix shape: {N_days} days x {N_stocks} stocks")
...
if __name__ == "__main__":
    ...
    print("Initializing Vectorized Train Environment...")
    env = DummyVecEnv([lambda: RLTraderEnvV4(train_dates, env_data, is_eval=False)])
    print("Training RL Model...")
    model = PPO("MlpPolicy", env, verbose=1, learning_rate=0.0003, ent_coef=0.01, n_steps=1024, batch_size=64)
    model.learn(total_timesteps=80000)
    model.save("/root/.openclaw/workspace-gemini-assistant/rl_trader/ppo_strategy/adaptive_ppo_v4_ram_model")
